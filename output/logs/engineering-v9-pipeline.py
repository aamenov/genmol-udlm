...
common = ['--protocol', 'experiments/udlm/protocols/engineering_v9_objectives.json', '--output-root', 'output/udlm/engineering_v9']
launch = subprocess.run([sys.executable, '-u', 'scripts/udlm/launch_exploration.py', *common, '--log-root', 'output/logs/engineering_v9'])
...
report = subprocess.run([sys.executable, '-u', 'scripts/udlm/report_exploration.py', *common, '--report-dir', 'output/udlm/engineering_v9_reports/complete'], env=report_env)
...
with open('output/logs/engineering-v9-pipeline-status.json', 'x') as stream:
