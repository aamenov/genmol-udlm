...
common = ['--protocol', 'experiments/udlm/protocols/engineering_v6.json', '--output-root', 'output/udlm/engineering_v6']
launch = subprocess.run([sys.executable, '-u', 'scripts/udlm/launch_exploration.py', *common, '--log-root', 'output/logs/engineering_v6'])
...
report = subprocess.run([sys.executable, '-u', 'scripts/udlm/report_exploration.py', *common, '--report-dir', 'output/udlm/engineering_v6_reports/complete'], env=report_env)
...
with open('output/logs/engineering-v6-pipeline-status.json', 'x') as stream:
