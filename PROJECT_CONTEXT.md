...
Snapshot: 2026-09-06, after the registered W=1 optimization screens and while
preparing the selection-bound 1,000-update R/S/E framework. Recheck Git, live
...
- The user authorizes up to four GPUs without another permission request.
...
1. Finish and independently review R4. Run focused tests, the exact-worktree
   ...
   `git diff --check`. Commit and push R4 to `aamenov/genmol-udlm`.
2. Use the CPU-only preparer to materialize the three configs for the chosen
   world size as the only R5 change. Review, test, commit, and push.
3. Use the CPU-only preparer/verifier to freeze the registry as the only R6
   change. Review, test, commit, and push.
4. Immediately before launch, inspect GPU utilization/memory/processes. If the
   requested count is eligible, launch only the next registered R/S/E member in
   detached tmux. Validate every receipt before advancing. Validate the terminal
   E panel while HEAD is still the exact clean, pushed R6 revision.
