...
if [[ "${2:-}" != --inside ]]; then
    ...
    if tmux has-session -t genmol-udlm-followthrough-learning 2>/dev/null; then
        echo 'Existing learning-curve session retained; refusing duplicate launch.' >&2
        exit 1
    fi
    exec tmux new-session -d -s genmol-udlm-followthrough-learning -c "$study_root" \
        "bash scripts/udlm/run_followthrough_learning.sh $study_arm --inside"
fi
[[ -n "${TMUX:-}" ]] || { echo 'Execution requires tmux.' >&2; exit 1; }
mkdir -p output/logs
set -o noclobber
exec > output/logs/followthrough-learning-pipeline.log 2>&1
