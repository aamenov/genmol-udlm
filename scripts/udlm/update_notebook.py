...
# Stages 20.1--20.5 are generated by ``stage_cells`` below, and Stage 20.9 is
# generated by ``_selection_bound_scale_up_cells``. Stages 20.6--20.8 were
# added as separately reviewed teaching material and must pass through this
# updater unchanged (apart from the notebook-wide output reset).
...
# USER SETTING: request 1--4 GPUs; physical IDs are selected dynamically.
NUM_GPUS: int = 1

if type(NUM_GPUS) is not int or not 1 <= NUM_GPUS <= 4:
    raise ValueError('NUM_GPUS must be an integer from 1 through 4.')

print(f'Requested GPU count: {NUM_GPUS} (hard ceiling: 4)')
"""
...
stage20_future_pilot_plan = {{
    "status": "w1_screens_completed_scale_up_registry_required",
    "execution_authority": {{
        ...
        "scale_up_registry_required": True,
    }},
...
assert stage20_screen_authority["scale_up_registry_required"] is True
...
stage20_decision_gate_report_rows = [
    ...
    *[
    ...
    (
        ...
        "the screen decisions alone do not authorize scale-up. A separate "
        "selection-bound registry must freeze matched R/S/E at 1000 updates. "
        "Only afterward, seed 1100 x 32 is an ineligible decode diagnostic. "
...
will use one GPU: the selected full-size E-A1 topology already completed 500
updates at W=1, making 1,000 updates a controlled twofold increase without also
introducing an untested multi-process topology.
...
utilization strictly below 10%, at least 30,000 MiB free, and non-prohibited
compute mode; up to four may be requested without hard-coding IDs.
...
request a GPU count from one through four; physical IDs are selected dynamically.

## What the next code cell does

The next cell defines `NUM_GPUS` as an integer from 1 through 4. It is a count, not a
...
The validation rejects zero, values above four, floats, strings, and Booleans.
...
3. Why is the four-GPU ceiling distinct from the paper's eight-GPU setup?
...
- The W=1 E-only 100-update scheduler screen selected E-L1; the subsequent two
  fresh 500-update conditioning arms selected E-A1. Both decisions used only a
  frozen denoising panel. Their committed envelopes contain no generation
  metrics and no final seeds. The next rung is a separately registered,
  selection-bound matched R/S/E panel at 1,000 updates per arm. Only after all
  three training receipts pass may seed 1100 x 32 requests run as an ineligible
  decode diagnostic; only seeds 1000/1001 x 256 requests at 128 NFE may select.
...
def update_notebook(source: Path, destination: Path):
    ...
    _validate_stage20_cells(
        notebook["cells"], [*replacement_cells, *late_stage20_cells]
    )
    ...
    late_stage20_ids = {cell["id"] for cell in late_stage20_cells}
    notebook["cells"] = [
    ...
    notebook["cells"][late_insert_at:late_insert_at] = late_stage20_cells
